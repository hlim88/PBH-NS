...
main(int argc, char **argv)
{
  ...
  if(argerr){
    fprintf(stderr," fluid_init\n");
    fprintf(stderr,"Usage: fluid_init\n");
  ...
  if(optind<argc){
     ...
     printf("fluid_init: Enter name of parameter file: ");
     fflush(stdout);
     scanf("%s",param_file);
  }
  if((fp=fopen(param_file,"r"))==NULL){
    fprintf(stderr,"fluid_init: Unable to open parameter file %s\n",param_file);
    exit(0);
  }
  fclose(fp);
  ...
  if(!check_params_attribs(Nr0,set_Nr0,epsdis,set_epsdis,epsiter,set_epsiter,epsiterid,set_epsiterid,fout,set_fout,in_file,set_in_file,iter,set_iter,lambda,set_lambda,level,set_level,maxstep,set_maxstep,maxstepid,set_maxstepid,out_file,set_out_file,output,set_output,rmax,set_rmax,rmin,set_rmin,s_step,set_s_step,ser,set_ser,start_t,set_start_t,tag,set_tag,trace,set_trace,out_gf,set_out_gf)){
    fprintf(stderr,"fluid_init: Unable to continue.\n");
    exit(0);
  }
  ...
  if(!set_epsiterid)
    epsiterid=epsiter;
  initializer(q1_np1,q1_n,q2_np1,q2_n,q3_np1,q3_n,p1_np1,p1_n,p2_np1,p2_n,p3_np1,p3_n,source1_n,source2_n,source3_n,flux1_n,flux2_n,flux3_n,fluxAlt1_n,fluxAlt2_n,fluxAlt3_n,alpha_n,a_n,lats[0].shape[0],r);
